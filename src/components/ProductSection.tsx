...
const ProductSection = () => {
  ...
  const processSteps = [{
    ...
    details: ["Full product marketing review with a diagnostic score and roadmap", "Customer interviews, competitor analysis", "Positioning workshops, strategy call, and a source-of-truth document"]
  }, {
    number: "02",
    title: "Content & Outbound Build",
    subtitle: "Execution",
    description: "Case studies, cold emails, CRM workflows—done for you.",
    details: ["Foundation workshops with analytics setup & reporting", "Project management with easy credit card payments", "No contracts, quotes, or billable hours"]
  }, {
    number: "03",
    title: "Launch & Iterate",
    subtitle: "Optimization",
    description: "We run experiments, track wins, and help you double down.",
    details: ["CRO & A/B Testing with data-driven optimization", "GTM launches, seasonal pages, PH launches", "SEO content with MoFu and BoFu strategy"]
  }];
  const handleEmailSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log("Strategy call email:", email);
  };
  return <section className="py-20 bg-section" id="process">
      <div className="container mx-auto max-w-6xl px-4">
        ...
        <div className="space-y-8 mb-16">
          {processSteps.map((step, index) => <div key={index} className="grid lg:grid-cols-3 gap-8 items-start">
              {/* Step Number */}
              ...
              <div className="lg:col-span-2">
                ...
                <ul className="space-y-3">
                  {step.details.map((detail, detailIndex) => <li key={detailIndex} className="flex items-start gap-3">
        ...
        <div className="bg-gradient-secondary rounded-2xl p-8 md:p-12 mb-16 border border-white/10">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <cite className="text-hero-foreground/70 not-italic">Saviour Ikegwuru, Founder</cite>
          </div>
        </div>
        ...
        <div className="bg-hero/50 backdrop-blur-sm rounded-2xl p-8 md:p-12 text-center border border-white/10">
          ...
          <form onSubmit={handleEmailSubmit} className="max-w-md mx-auto">
            <div className="flex gap-2">
              <Input type="email" placeholder="Enter your email for instant access" value={email} onChange={e => setEmail(e.target.value)} className="flex-1 bg-white/10 border-white/20 text-hero-foreground placeholder:text-hero-foreground/60" required />
